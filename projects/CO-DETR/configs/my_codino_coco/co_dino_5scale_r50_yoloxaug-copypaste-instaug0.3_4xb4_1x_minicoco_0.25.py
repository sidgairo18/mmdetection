...
# Inner dataset pipeline (loading + basic aug)
load_pipeline_lcp = [
...
# Outer augmentation pipeline (MultiImageMixDataset)
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        datasets=[
            ...
            dict(
                type='MultiImageMixDataset',
                pipeline=train_pipeline_yolox,
                dataset=dict(
                    type=dataset_type,
                    data_root=data_root,
                    ann_file='annotations/instances_train2017_minicoco_0.25.json',
                    data_prefix=dict(img='train2017/'),
                    pipeline=load_pipeline_yolox,
                    filter_cfg=dict(filter_empty_gt=False, min_size=32),
                    backend_args=backend_args,
                )
            )
