...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        dataset=dict(
            ...
            ann_file='annotations/instances_train2017_minicoco_0.25.json',
...
# --- Visualizer: Local + W&B backends ---
visualizer = dict(
    _delete_=True,                      # replace inherited visualizer if any
    type='DetLocalVisualizer',
    vis_backends=[
        dict(type='LocalVisBackend'),   # keep local images/media
        dict(
            type='WandbVisBackend',
            init_kwargs=dict(
                project='codetr_minicoco',       # <-- set your W&B project
                entity='sidgairo18-saarland-informatics-campus'    # <-- set your user/team
                # name/group/tags will be auto-set by WandbAutoMetaHook
                #settings=dict(x_disable_stats=True)  # disable system stats collection
            ),
            # Nice: define summaries & axes so plots look right
            define_metric_cfg=[
                dict(name='coco/bbox_mAP', summary='max'),
                dict(name='coco/segm_mAP', summary='max'),
                dict(name='train/loss', summary='min', step_metric='iter'),
                dict(name='lr', step_metric='iter'),
            ],
            # Optional niceties:
            # log_code_name='source',
            # watch_kwargs=dict(log='gradients')  # can be 'all' but costs memory/time
        )
    ],
    name='visualizer'
)
...
# --- Hooks ---
default_hooks = dict(
    # keep your intervals; add logger + visualization + best checkpoint
    logger=dict(type='LoggerHook', interval=50),
    #visualization=dict(type='DetVisualizationHook', draw=True, interval=100, show=False),
    visualization=dict(type='DetVisualizationHook', draw=False),
    checkpoint=dict(by_epoch=True, interval=1, max_keep_ckpts=3, save_best='coco/bbox_mAP')
)

# Register our two custom hooks by type (registry finds them via the import in __init__.py)
'''
custom_hooks = [
